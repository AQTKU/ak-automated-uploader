...
export default class SP extends Tracker {
    apiKey = '';
    override name = 'SP';
    ...
    // Seedpool custom category logic
    private isSports(title: string): boolean {
        const patterns = [
    ...
    applyRelease(release: Release) {
        // Basic UNIT3D mapping (same as LST)
        this.setOption('resolutionId', release.resolution ?? 'Other');
        this.setOption('typeId', release.fullDisc ? 'Full Disc'
            : release.remux ? 'Remux'
            : release.source?.endsWith('WEB-DL') ? 'WEB-DL'
            : release.source?.endsWith('WEBRip') ? 'WEBRip'
            : release.source === 'HDTV' ? 'HDTV'
            : 'Encode');
        ...
        // Anime override
        if (release.malId && release.category === 'tv') {
            this.setOption('categoryId', '6');
        }

        // Sports override
        if (this.isSports(release.title)) {
            this.setOption('categoryId', '8');
        }

        // Title formatting
        this.data.name = release.format('{title aka} {year} {resolution} {source} -{group}');
    }
