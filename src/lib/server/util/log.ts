...
export function log(message: string, color_: 'tomato' | 'khaki' | 'aquamarine' | 'lightgrey' = 'lightgrey') {
    ...
    switch (color_) {
        case 'tomato': case 'khaki':
            process.stderr.write(prefix + color(color_, 'ansi-16m') + message + suffix + EOL);
            break;
        case 'aquamarine': default:
            process.stdout.write(prefix + color(color_, 'ansi-16m') + message + suffix + EOL);
            break;
    }
}